...
As input, FLOP takes the data matrix, the BIC penalty parameter (we
recommend `2.0` as a default choice) and either a `timeout` (in seconds)
or the number of ILS restarts to control how long the search runs.

The output of FLOP is a CPDAG encoded with an adjacency matrix whose
