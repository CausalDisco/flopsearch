...
impl Bic {
    pub fn new(data: &DMatrix<f64>, lambda: f64) -> Self {
        Self {
            n: data.nrows(),
            lognlambda: (data.nrows() as f64).ln() * lambda,
            nroot: (data.nrows() as f64).powf(-lambda / (2.0 * (data.nrows() as f64))),
    ...
    pub fn from_cov(n: usize, cov: DMatrix<f64>, lambda: f64) -> Self {
        Self {
            n,
            lognlambda: (n as f64).ln() * lambda,
            nroot: (n as f64).powf(-lambda / (2.0 * (n as f64))),
            cov,
        }
    }
    ...
    // TODO
    pub fn local_score_plus_if_improved(
        &self,
        v: usize,
        old_local: &LocalScore,
        r: usize,
    ) -> Result<Option<LocalScore>, ScoreError> {
        let num_parents = old_local.parents.len() + 1;
        let mut parents_v_r = Vec::with_capacity(num_parents + 1);
        parents_v_r.extend_from_slice(&old_local.parents);
        parents_v_r.push(v);
        ...
        let old_std_var = old_local.chol.get_bottom_right();
        let std_var = new_chol.get_bottom_right();

        if std_var < old_std_var * self.nroot {
            let mut new_parents = parents_v_r;
            new_parents.pop();
            new_parents[num_parents - 1] = r;
            Ok(Some(LocalScore {
    ...
    // TODO
    pub fn local_score_minus_if_improved(
        &self,
        _v: usize,
        old_local: &LocalScore,
        r: usize,
